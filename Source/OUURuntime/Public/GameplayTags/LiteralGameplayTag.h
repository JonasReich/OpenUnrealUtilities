...
struct TLiteralGameplayTag : FLiteralGameplayTagBase
{
	...
	static_assert(TIsDerivedFrom<RootTagType, FLiteralGameplayTagRoot>::Value,
...
struct TConditionalNativeTag<OwningType, true>
{
	...
	{
		return OwningType::GetInstance().NativeInstance.GetTag();
	}
};
 ...
 * bAutoAddNativeTag: If yes, all the tags inside are added as native tags to the gameplay tags manager similar to how FAutoConsoleObject registers with IConsoleManager.
 */
...
/**
 * Put this into a cpp file so the literal gameplay tag root container is only defined once for multiple translation units.
 * This prevents linker errors due to multiple definitions.
 */
#define DEFINE_OUU_GAMEPLAY_TAGS(RootName) \
	RootName RootName ## _Instance;
