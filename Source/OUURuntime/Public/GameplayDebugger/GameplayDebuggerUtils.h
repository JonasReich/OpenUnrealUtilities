...
namespace GameplayDebuggerUtils
{
	...
	FString OUURUNTIME_API WrapStringToWidth(const FString& InString, FGameplayDebuggerCanvasContext& CanvasContext, float TargetWidth);
	 ...
	 */
	bool OUURUNTIME_API TryWrapStringToWidth(const FString& InString, FString& OutString, FGameplayDebuggerCanvasContext& CanvasContext, float TargetWidth);

	/**
	 * @returns a copy of the name that has class default object prefix and class suffix removed to make names a bit more readable.
	 * Obviously, this loses some information that allows differentiating between CDOs, classes, etc. so use with care!
	 */
