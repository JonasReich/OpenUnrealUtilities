...
public:
	 ...
	 * As opposed to the APlayerController::ProjectWorldLocationToScreen() this function does not rely on the last
	 * rendered frame, but can predict arbitrary perspectives based on the current camera component settings.
	 */
	 ...
	 * As opposed to the APlayerController::DeprojectScreenPositionToWorld() this function does not rely on the last
	 * rendered frame, but can predict arbitrary perspectives based on the current camera component settings.
	 */
	UFUNCTION(BlueprintCallable, Category = "Open Unreal Utilities|Scene Projection")
