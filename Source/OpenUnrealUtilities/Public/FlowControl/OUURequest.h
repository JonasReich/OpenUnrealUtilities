...
class OPENUNREALUTILITIES_API UOUURequest : public UObject
{
	GENERATED_BODY()
public:
...
public:
	...
	/**
	 * Get a list of all pending requests in the queue.
	 * Items are sorted by the time the individual requests were raised.
	 */
	UFUNCTION(BlueprintCallable)
	TArray<UOUURequest*> GetPendingRequests();
...
private:
	...
	UFUNCTION()
	void HandleRequestRaised(UOUURequest* Request) const;

	// React to one of the requests that were created in the queue being completed.
	UFUNCTION()
	void HandleRequestCompleted(UOUURequest* Request, EOUURequestState State) const;
};
