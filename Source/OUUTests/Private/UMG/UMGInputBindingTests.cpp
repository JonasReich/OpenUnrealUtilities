...
void FUMGInputActionBindingSpec::Define()
{
	BeforeEach([this]() {
		...
		Widget = Cast<UUMGInputBindingTestWidget>(UUserWidget::CreateWidgetInstance(
			*TestWorld->PlayerController,
			UUMGInputBindingTestWidget::StaticClass(),
			NAME_None));
	...
	Describe("CreateUMGInputActionBindingStack", [this]() {
		...
		It("should throw an error and return nullptr if the provided widget is invalid", [this]() {
			AddExpectedError(
				"Could not create UMGInputActionBindingStack with invalid OwningWidget!",
				EAutomationExpectedErrorFlags::Exact,
				1);
			UUMGInputActionBindingStack* ResultStack =
				UUMGInputActionBindingStack::CreateUMGInputActionBindingStack(nullptr);
	...
	Describe("Binding a simple key down event", [this]() {
		BeforeEach([this]() {
			...
			FUMGInputAction
				ActionOne{TestActionName, EUMGInputActionKeyEvent::KeyDown, EUMGInputActionKeyEventConsumeMode::Same};
			Stack->BindAction(
				ActionOne,
				CreateDynamic(FUMGInputActionDelegate, Widget, &UUMGInputBindingTestWidget::HandleInputActionEventOne));
		});

		It("should result in a single bindings being present with the target object",
		   [this]() { TestEqual("Number of bindings", Stack->GetNumBindingsToObject(Widget), 1); });
	...
	Describe("Binding two events with different keys", [this]() {
		BeforeEach([this]() {
			...
			FUMGInputAction
				ActionOne{FirstActionName, EUMGInputActionKeyEvent::KeyDown, EUMGInputActionKeyEventConsumeMode::Same};
			Stack->BindAction(
				ActionOne,
				CreateDynamic(FUMGInputActionDelegate, Widget, &UUMGInputBindingTestWidget::HandleInputActionEventOne));
			...
			FUMGInputAction
				ActionTwo{SecondActionName, EUMGInputActionKeyEvent::KeyDown, EUMGInputActionKeyEventConsumeMode::Same};
			Stack->BindAction(
				ActionTwo,
				CreateDynamic(FUMGInputActionDelegate, Widget, &UUMGInputBindingTestWidget::HandleInputActionEventTwo));
		});

		It("should result in two bindings being present with the target object",
		   [this]() { TestEqual("Number of bindings", Stack->GetNumBindingsToObject(Widget), 2); });
	...
	Describe("Marking an action as bIsOneshot", [this]() {
		BeforeEach([this]() {
			const FName FirstActionName = "FirstAction";
			PlayerInput->AddActionMapping(FInputActionKeyMapping(FirstActionName, EKeys::One));
			FUMGInputAction ActionOne{
				FirstActionName,
				EUMGInputActionKeyEvent::KeyDown,
				EUMGInputActionKeyEventConsumeMode::Same,
				/*bIsOneshot*/ true};
			Stack->BindAction(
				ActionOne,
				CreateDynamic(FUMGInputActionDelegate, Widget, &UUMGInputBindingTestWidget::HandleInputActionEventOne));
	...
	It("RemoveBindings should remove the bindings of the same type", [this]() {
		...
		Stack->BindAction(
			ActionTwo,
			CreateDynamic(FUMGInputActionDelegate, Widget, &UUMGInputBindingTestWidget::HandleInputActionEventThree));
	...
	It("RemoveBindingByObject should remove only the bindings to the target object", [this]() {
		UUMGInputBindingTestWidget* SecondWidget = CreateWidget<UUMGInputBindingTestWidget>(
			TestWorld->PlayerController,
			UUMGInputBindingTestWidget::StaticClass(),
			NAME_None);
		...
		Stack->BindAction(
			ActionOne,
			CreateDynamic(
				FUMGInputActionDelegate,
				SecondWidget,
				&UUMGInputBindingTestWidget::HandleInputActionEventOne));
		...
		Stack->BindAction(
			...
			CreateDynamic(
				FUMGInputActionDelegate,
				SecondWidget,
				&UUMGInputBindingTestWidget::HandleInputActionEventTwo));
