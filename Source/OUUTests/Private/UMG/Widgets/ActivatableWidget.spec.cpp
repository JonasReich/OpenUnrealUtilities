...
void FActivatableWidgetSpec::Define()
{
	...
	Describe("Activate", [this]() {
		...
		It("should change the visibility from Hidden to Visible if the visibility properties are set accordingly",
		   [this]() {
	...
	Describe("ForceUpdateVisibility", [this]() {
		It("should not change the visibility after activation, as it should already be set to the correct value",
		   [this]() {
			   Widget->Activate();
