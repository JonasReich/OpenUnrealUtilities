...
void FLayerStackWidgetSpec::Define()
{
	...
	Describe("NativeOnInitialized", [this]() {
		...
		It("checks if there are any widgets in the stack overlay and throws a warning if it doesn't have any layer "
		   "children",
		   [this]() {
		...
		It("doesn't throw any error if the widget stack layer widget is already set and filled with at least one layer",
		   [this]() { CreateWidget<UTestUOUULayerStackWidget>(TestWorld->PlayerController); });
	});
	...
	Describe("", [this]() {
		...
		It("IsLinkedStackHead should throw an error and return false on a stack that was not added to a linked stack",
		   [this]() {
		...
		Describe("", [this]() {
			BeforeEach([this]() { StackWidget->StartNewLinkedStack(); });
			...
			Describe("InsertToLinkedStackAbove", [this]() {
				...
				Describe("to a linked stack that already has two stack elements", [this]() {
					BeforeEach([this]() { SecondStackWidget->InsertToLinkedStackAbove(StackWidget); });
					...
					It("should the newly added stack as linked stack head if it's added above the old head", [this]() {
						...
						SPEC_TEST_EQUAL(
							StackWidget->GetLinkedStackHead(),
							Cast<UOUULayerStackWidget>(ThirdStackWidget));
					});

					It("should not be the head and the head should remain the same if it's added above the root",
					   [this]() {
			...
			Describe("InsertToLinkedStackBelow", [this]() {
				...
				It("should still be able to point to the correct stack head when followed up by "
				   "InsertToLinkedStackAbove",
				   [this]() {
					   SecondStackWidget->InsertToLinkedStackBelow(StackWidget);
					   ThirdStackWidget->InsertToLinkedStackAbove(StackWidget);
					   SPEC_TEST_TRUE(ThirdStackWidget->IsLinkedStackHead());
					   SPEC_TEST_EQUAL(
						   SecondStackWidget->GetLinkedStackHead(),
						   Cast<UOUULayerStackWidget>(ThirdStackWidget));
				   });
			});

			Describe("RemoveFromLinkedStack", [this]() {
				It("should remove the stack from the viewport but do nothing else when called on a single stack that "
				   "is not linked",
				   [this]() {
					   StackWidget->RemoveFromLinkedStack();
					   // no simple way to simulate/verify that it was removed from the viewport, so we just run this
					   // to check for errors thrown
				   });
				...
				Describe("", [this]() {
					...
					It("should remove a linked stack head and turn the stack below into the new stack head", [this]() {
						...
						SPEC_TEST_TRUE(SecondStackWidget->IsLinkedStackHead());
						SPEC_TEST_EQUAL(
							ThirdStackWidget->GetLinkedStackHead(),
							Cast<UOUULayerStackWidget>(SecondStackWidget));
						SPEC_TEST_NULL(SecondStackWidget->GetStackAbove());
					});

					It("should remove stack from the middle of a linked stack chain without invalidating the stack "
					   "head",
					   [this]() {
						   ...
						   SPEC_TEST_EQUAL(
							   ThirdStackWidget->GetLinkedStackHead(),
							   Cast<UOUULayerStackWidget>(StackWidget));
			...
			Describe("NativeTick", [this]() {
				It("should call UpdateLayer on all layers inside a stack", [this]() {
					...
					SPEC_TEST_ARRAYS_EQUAL(
						UTestUOUULayerStackWidget::LayersAboveUpdatedLayers,
						ExpectedLayersAboveUpdatedLayers);
				});
				...
				It("should call UpdateLayer on layers across linked widget stacks", [this]() {
					...
					const TArray<const UOUULayerWidget*> ExpectedUpdatedLayers = {
						StackWidget->TopLayer,
						StackWidget->MiddleLayer,
						StackWidget->BottomLayer,
						SecondStackWidget->TopLayer,
						SecondStackWidget->MiddleLayer,
						SecondStackWidget->BottomLayer};
					SPEC_TEST_ARRAYS_EQUAL(UTestUOUULayerStackWidget::UpdatedLayers, ExpectedUpdatedLayers);
					const TArray<const UOUULayerWidget*> ExpectedLayersAboveUpdatedLayers = {
						nullptr,
						StackWidget->TopLayer,
				...
				AfterEach([this]() { UTestUOUULayerStackWidget::ClearUpdatedLayers(); });
			});
		});
	});
