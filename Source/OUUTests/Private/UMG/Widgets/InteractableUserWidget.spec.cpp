...
BEGIN_DEFINE_SPEC(
	FInteractableUserWidgetSpec,
	"OpenUnrealUtilities.UMG.Widgets.InteractableUserWidget",
...
void FInteractableUserWidgetSpec::Define()
{
	...
	Describe("TryResetUserFocusTo", [this]() {
		It("should fail when called on nullptr",
		   [this]() { SPEC_TEST_FALSE(IUserFocusResetableWidget::TryResetUserFocusTo(nullptr)); });

		It("should fail to set the focus when the target widget is a focusable button, because it does not implement "
		   "the interface",
		   [this]() {
		...
		It("should fail to set the focus when the target is an empty UUserWidget that is not focusable itself",
		   [this]() {
		...
		It("should successfully set the focus when the target is an empty UUserWidget that is focusable itself",
		   [this]() {
		...
		Describe(
			"called on a UUserWidget that contains another UUserWidget which implements IUserFocusResetableWidget",
			[this]() {
				It("should successfully set the focus if ResetUserFocus_Implementation in the nested widget returns "
				   "true",
				   [this]() {
					   UHorizontalBox* HorizontalBox = WidgetTree->ConstructWidget<UHorizontalBox>();
					   WidgetTree->RootWidget = HorizontalBox;
				...
				It("should fail to set the focus if ResetUserFocus_Implementation in the nested widget returns false",
				   [this]() {
					   UHorizontalBox* HorizontalBox = WidgetTree->ConstructWidget<UHorizontalBox>();
					   WidgetTree->RootWidget = HorizontalBox;
					   UMockFocusResetable_False* NestedWidget =
						   WidgetTree->ConstructWidget<UMockFocusResetable_False>();
					   HorizontalBox->AddChildToHorizontalBox(NestedWidget);
					   auto SlateWidget = Widget->TakeWidget();
					   SPEC_TEST_FALSE(IUserFocusResetableWidget::TryResetUserFocusTo(Widget));
				   });
