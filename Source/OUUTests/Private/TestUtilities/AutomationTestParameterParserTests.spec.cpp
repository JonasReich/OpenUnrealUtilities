...
void LexFromString(EParameterParserTestEnum_Parseable& Out, const FString& InString)
{
	if (InString == "Alpha")
	{
		Out = EParameterParserTestEnum_Parseable::Alpha;
	}
	else if (InString == "Beta")
	{
		Out = EParameterParserTestEnum_Parseable::Beta;
	}
	else if (InString == "Gamma")
	{
...
BEGIN_DEFINE_SPEC(
	FAutomationTestParameterParserSpec,
	"OpenUnrealUtilities.TestUtilities.ParameterParser",
...
void FAutomationTestParameterParserSpec::Define()
{
	...
	Describe("GetValue", [this]() {
		It("should parse parameter values based on the index", [this]() {
			const FAutomationTestParameterParser Parser("0|1|2|42");
		...
		It("should parse string parameters", [this]() {
			const FAutomationTestParameterParser Parser("0|My test string|2");
			TestEqual("Result string", Parser.GetValue<FString>(1), "My test string");
		});

		It("should parse boolean parameters", [this]() {
			const FAutomationTestParameterParser Parser("0|1|true|false");
		...
		It("should parse enum parameters from integers if LexFromString is NOT defined", [this]() {
			const FAutomationTestParameterParser Parser("1|0|2");
		...
		It("should parse enum parameters from strings if LexFromString IS defined", [this]() {
			const FAutomationTestParameterParser Parser("Beta|Alpha|0|Gamma");
			TestEqual(
				"Parameter #1",
				Parser.GetValue<EParameterParserTestEnum_Parseable>(0),
				EParameterParserTestEnum_Parseable::Beta);
			TestEqual(
				"Parameter #2",
				Parser.GetValue<EParameterParserTestEnum_Parseable>(1),
				EParameterParserTestEnum_Parseable::Alpha);
			TestEqual(
				"Parameter #3",
				Parser.GetValue<EParameterParserTestEnum_Parseable>(2),
				EParameterParserTestEnum_Parseable::Invalid);
			TestEqual(
				"Parameter #4",
				Parser.GetValue<EParameterParserTestEnum_Parseable>(3),
				EParameterParserTestEnum_Parseable::Gamma);
		});
	});

	Describe("GetArrayValue", [this]() {
		It("should return an array of elements that were separated by ';'", [this]() {
			const FAutomationTestParameterParser Parser("1;2;3|4;5;6|7;8;9");
			TestEqual("Result array", Parser.GetArrayValue<int32>(1), {4, 5, 6});
		});

		It("should return an array of elements that were separated by custom delimiter if specified in constructor",
		   [this]() {
			   const FAutomationTestParameterParser Parser("1@2@3o4@5@6o7@8@9", "o", "@");
