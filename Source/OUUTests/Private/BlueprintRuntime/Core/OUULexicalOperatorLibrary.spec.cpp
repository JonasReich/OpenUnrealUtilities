...
BEGIN_DEFINE_SPEC(FOUULexicalOperatorLibrarySpec, "OpenUnrealUtilities.BlueprintRuntime.Core.OUULexicalOperatorLibrary", DEFAULT_OUU_TEST_FLAGS)
	template <typename TargetType>
	using TOperatorType = TFunction<bool(const TargetType&, const TargetType&)>;

	template <typename TargetType>
	void DescribeSingleOperatorTestCases(TargetType A, TargetType B, TOperatorType<TargetType> OperatorFunc, bool AB_Result, bool AA_Result, bool BA_Result)
	{
	...
	template <typename TargetType>
	void DescribeAllOperatorsForType(TargetType A, TargetType B,
...
void FOUULexicalOperatorLibrarySpec::Define()
{
	...
	for (int32 i = 0; i < TestStrings.Num() - 2; i++)
	{
		Describe("String", [=, this]()
		{
			...
			DescribeAllOperatorsForType<FString>(AString, BString,
		...
		Describe("Name", [=, this]()
		{
			...
			DescribeAllOperatorsForType<FName>(AName, BName,
